...
class TrafficSimulator:
    ...
    def reset_network(self, ssid, auth, enc):
        # Set anomaly threshold based on network encryption/auth state
        is_unsecured = (auth == 'Open' or enc == 'None' or auth == '' or enc == '')
        if is_unsecured:
            self.anomaly_threshold = 0.60 # Higher anomaly rate
        else:
            self.anomaly_threshold = 0.95 # Highly secure rate
            
        # Reset counters
        self.total_packets_checked = random.randint(1500, 5000)
        self.total_threats_detected = 0
        self.threat_counts = {
            'Normal': self.total_packets_checked,
            'DDoS Attack': 0,
            'SQL Injection': 0,
            'Port Scan': 0,
            'Brute Force': 0
        ...
        # Seed with initial 50 packets matching the new network context
        self._generate_initial_data(50)
