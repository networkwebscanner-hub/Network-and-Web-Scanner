...
def get_stats():
    try:
        ...
        try:
            # Safe psutil queries for system health telemetry
            import psutil
            import os
            ...
            try:
                disk_val = psutil.disk_usage(disk_path).percent
            except Exception:
                disk_val = 42.3
            ...
            # Safe fallback if psutil is not available
            import random
            stats["system"] = {
                "cpu": round(random.uniform(12.5, 28.4), 1),
                "memory": round(random.uniform(58.2, 64.9), 1),
                "disk": 42.3
            }
...
def select_network():
    try:
        ...
        # Reset and re-seed the simulator state
        simulator.reset_network(ssid, auth, enc)
...
def local_backend():
    ...
    try:
        if action == "loginUser":
            ...
            print(f"==========================================\n")
            return jsonify({"success": True, "userCode": user["user_code"], "otp": otp})
            ...
            email = payload.get("email", "").strip()
            password = payload.get("newPassword") or payload.get("password") or ""
