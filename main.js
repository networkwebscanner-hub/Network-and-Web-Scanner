...
async function callBackend(payload) {
    ...
    const hasAppsScript = typeof APPS_SCRIPT_URL !== 'undefined' && APPS_SCRIPT_URL && !APPS_SCRIPT_URL.includes("REPLACE_THIS") && !APPS_SCRIPT_URL.includes("YOUR_DEPLOYED_GOOGLE");
    ...
    if (isOnline && hasAppsScript) {
        ...
        endpoint = 'http://127.0.0.1:5000/api/backend';
    }
    ...
    try {
        ...
        // Dynamic fallback: If relative API fails (e.g. deployed online without Python hosting), try local laptop's Flask server.
        if (endpoint === '/api/backend') {
            console.log("Relative backend unavailable. Retrying connection on local Flask database...");
            try {
                const retryRes = await fetch('http://127.0.0.1:5000/api/backend', {
                    method: "POST",
                ...
                console.error("Local database fallback failed. Shifting to simulation:", retryErr);
            }
        ...
        } else if (endpoint === APPS_SCRIPT_URL) {
            try {
                const retryRes = await fetch('http://127.0.0.1:5000/api/backend', {
                    method: "POST",
                    headers: { "Content-Type": "application/json" },
        ...
        if (endpoint === 'http://127.0.0.1:5000/api/backend') {
            return handleOfflineSimulation(payload);
        }
